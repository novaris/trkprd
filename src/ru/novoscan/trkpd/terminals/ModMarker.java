...
public class ModMarker extends Terminal {
	...
	private static final Pattern patternMarker = Pattern
			.compile("^\\$GM" + "(\\d{1})" // Тип данных навигации для 2 типа -
											// GPS
					+ "(..)" // ИД пакета 2
					+ "(\\d{15})" // IMEI 3
					+ "T" + "(\\d{12})" // DDMMYYHHMMSS - дата время с секундами
										// 4
					+ "(N|S|_)" // 5
			...
			if (packetSize > maxPacketSize) {
				...
				if (m.matches()) {
					...
					logger.debug("Send reply : " + "__" + m.group(2)
							+ (char) (0x0d) + (char) (0x0a));
					oDs.writeBytes(
							"__" + m.group(2) + (char) (0x0d) + (char) (0x0a));
					oDs.flush();
					slog = "";
					packetSize = 0;
				} else {
