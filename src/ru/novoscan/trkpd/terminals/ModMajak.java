	...
	public ModMajak(DataInputStream iDs, DataOutputStream oDs,
		...
		while (true) {
			...
			if (navPacketType == 0x41) {
				...
				logger.debug(
						"Интервал пробуждения (0 - без сна) : " + packet[5]);
				logger.debug(
						"Единицы измерения инервала пробуждения (M - минуты, иначе часы) : "
								+ packet[6]);
				logger.debug(
						"Режим работы : " + Integer.toHexString(packet[7]));
				logger.debug("Интервал передачи : " + packet[8] + " сек");
				logger.debug("MCC : " + packet[9]);
				logger.debug("MNC : " + packet[10]);
				logger.debug("LAC : " + Integer
						.toHexString(((packet[11] << 8) + packet[12])));
				logger.debug("CID : " + Integer
				...
				String navDateTime = utl.getLastnCharacters(
						"000000" + String.valueOf((packet[19] << 16)
								+ (packet[20] << 8) + packet[21]),
						6)
						+ utl.getLastnCharacters(
								"000000" + String.valueOf((packet[16] << 16)
				...
				dasnLatitude = (double) packet[22]
						+ ((float) ((packet[23] << 12) + (packet[24] << 4)
								+ (packet[25] >> 4))) / (600000);
				dasnLongitude = (double) packet[26]
						+ ((float) ((packet[27] << 12) + (packet[28] << 4)
								+ (packet[29] >> 4))) / (600000);
				;
				logger.debug(
						"Широта (градусы) : " + String.valueOf(dasnLatitude));
				logger.debug(
						"Долгота (градусы) : " + String.valueOf(dasnLongitude));
